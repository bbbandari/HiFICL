...
class Dataset(DatasetBase):
    ...
    def __init__(self, data_cfg):
        ...
        if self.name == "vqav2":
            ...
            ds_init_args = dict(
                path=os.path.join(paths.testbed_dir, "data", "ok_vqa"),
        ...
        for _, batch in zip(
            ...
            for pred, context in zip(predictions, batch):
                ...
                prediction = postprocess_generation(
                    self.name,
                    pred,
                    ["\n", "Question", "Answer", "Image", "IMAGE:", "IMAGE: ", "Short"],
                )
    ...
    def eval_vqa(self, eval_cfg, lmm):
        ...
        # 如果没有任何数据被添加到metric，返回空结果
        if len(result) == 0:
            print(f"Warning: No predictions collected. num_skipped={num_skipped}, iterations={iterations}")
            return result, {}
        
        print(f"Evaluation completed: {len(result)} predictions collected, {num_skipped} batches skipped")
        return result, metric.compute()
