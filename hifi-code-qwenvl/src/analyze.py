...
def sort_runname_key(runname: str):
    ...
    try:
        ...
        # Check if query_samples is numeric and shot contains a number
        if query_samples.isdigit() and re.findall(r"\d+", shot):
            return (prefix, int(query_samples), int(re.findall(r"\d+", shot)[0]))
    except (ValueError, IndexError):
        pass
    # Return a default sort key for directories that don't match the expected format
    return (runname, 0, 0)
...
def main(cfg: DictConfig):
    ...
    # 如果 record 目录不存在，创建一个空字典
    if not os.path.exists(record_base_dir):
        os.makedirs(record_base_dir, exist_ok=True)
    record_dirs = {
            ...
            if os.path.isdir(ckpt_dir):
                for epoch_ckpt in os.listdir(ckpt_dir):
                    if epoch_ckpt not in meta_info:
                        ...
                        epoch_numbers = re.findall(r"\d+", epoch_ckpt)
                        if epoch_numbers:  # Only process if we found numbers
                            missing_epoch = int(epoch_numbers[0])
