...
class DatasetBase(ABC, metaclass=PostInitMeta):
    ...
    def get_prediction(
        ...
        **generation_args,
        ...
        try:
            result = model.generate(*args_to_generate, **generation_args)
            if result is None or len(result) == 0:
                logger.warning(f"Model.generate returned None or empty result")
            return result
        except Exception as e:
            logger.error(f"Exception in get_prediction: {type(e).__name__}: {str(e)}")
            if "out of memory" not in str(e):
                raise
            if not hasattr(self, "__num_skip_oom"):
                self.__num_skip_oom = 0
            if self.__num_skip_oom >= max_skip_oom:
                logger.error(f"Max skip OOM ({max_skip_oom}) reached, raising exception")
                raise
            else:
                self.__num_skip_oom += 1
                logger.warning(f"Skipping OOM sample {self.__num_skip_oom}/{max_skip_oom}")
    ...
    def train_dataloader(
        ...
        if self.cfg.num_shot > 0:
            # prepare example sampler (only needed when num_shot > 0)
