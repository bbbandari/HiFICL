...
def main(cfg: DictConfig):
    ...
    def save_when(epoch):
        # 如果设置了save_when_override，则从第一轮开始保存
        if hasattr(cfg, 'save_when_override') and cfg.save_when_override:
            return True
    ...
    if cfg.resume:
        ...
        # 如果目录为空，说明没有训练过，应该继续训练
        if len(exist_ckpt_epochs) == 0:
            print(f"Checkpoint directory exists but is empty, starting training for {runname}")
        else:
