...
class ShiftModel(pl.LightningModule):
    ...
    def forward(self, prefix_texts, query_texts, answers, images):
        ...
        # Prepare labels properly: only compute loss on the answer part
        if Strategy.LM_LOSS in self.strategy:
            # Start from input_ids and mask out padding positions first
            labels = query_inputs["input_ids"].clone()
            labels = labels.masked_fill(query_inputs["attention_mask"] == 0, -100)

            # For each sample, mask everything up to and including the separator pad_token
            pad_positions = (query_inputs["input_ids"] == pad_token_id).nonzero(as_tuple=True)

            for i in range(query_inputs["input_ids"].shape[0]):
                sample_pad_positions = pad_positions[1][pad_positions[0] == i]
                if len(sample_pad_positions) > 0:
                    # Mask prompt (query and separator), keep answer (including eos)
                    labels[i, : sample_pad_positions[-1] + 1] = -100  # ignore_index
        else:
            labels = None
    ...
    def configure_optimizers(self):
        ...
        # 调试信息：检查优化器参数组
        print(f"Optimizer groups count: {len(optim_groups)}")
        for i, group in enumerate(optim_groups):
            params_count = len(group.get("params", []))
            print(f"Group {i}: {params_count} parameters")
