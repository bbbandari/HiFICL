...
class ModelBase(nn.Module, ABC):
    ...
    def generate(
        ...
        **generate_args,
        ...
        # Ensure eos_token_id is not None or equal to pad_token_id to avoid immediate stopping
        eos_token_id = self.processor.tokenizer.eos_token_id
        pad_token_id = self.processor.tokenizer.pad_token_id
        
        # If pad_token_id equals eos_token_id, we need to handle this specially
        if pad_token_id == eos_token_id and eos_token_id is not None:
            # Remove eos_token_id from generation_args to prevent immediate stopping
            # But keep it for the actual stopping condition
            gen_args_copy = generate_args.copy()
            # Don't set pad_token_id in generation args, let the model handle it
            if 'pad_token_id' in gen_args_copy:
                del gen_args_copy['pad_token_id']
            generated_ids = self.model.generate(**inputs, **gen_args_copy)
        else:
            generated_ids = self.model.generate(**inputs, **generate_args)
        
        generated_ids = generated_ids[:, seq_len:]

        # Decode without skipping special tokens first, then manually clean them
        # This is because skip_special_tokens=True may filter out too much content
        raw_outputs = self.processor.batch_decode(generated_ids, skip_special_tokens=False)
        
        # Check what tokens were generated - use warning level for visibility
        if generated_ids.shape[-1] > 0:
            sample_tokens = generated_ids[0].cpu().tolist()[:10]  # First 10 tokens
            eos_token_id = self.processor.tokenizer.eos_token_id
            # Check if all tokens are eos_token_id or pad_token_id
            if eos_token_id is not None:
                all_eos = all(t == eos_token_id for t in sample_tokens)
                if all_eos:
                    logger.warning(f"WARNING: All generated tokens are EOS token ({eos_token_id}). Generated token IDs (first 10): {sample_tokens}")
            
            # Log raw decoded output for debugging
            if raw_outputs and any(len(out.strip()) > 0 for out in raw_outputs[:2]):
                logger.warning(f"Raw decoded outputs (skip_special_tokens=False) have content: {raw_outputs[0][:100] if len(raw_outputs[0]) > 100 else raw_outputs[0]}")
        
        # Manually remove special tokens that interfere with output
        # Keep only the actual text content, remove chat template tokens
        outputs = []
        for raw_out in raw_outputs:
            # Remove chat template special tokens but preserve content
            cleaned = raw_out
            ...
            # If the result is empty or only contains special characters, try to extract meaningful content
            if not cleaned or len(cleaned) < 2:
                # Look for any remaining text before special tokens
                text_match = re.search(r'^([^<]+)', raw_out)
                if text_match:
        ...
        if outputs is None or len(outputs) == 0:
            logger.warning(f"batch_decode returned empty: outputs={outputs}, generated_ids.shape={generated_ids.shape}")
        else:
            # Check if outputs are mostly empty or just whitespace
            non_empty_outputs = [out for out in outputs if out.strip()]
            if len(non_empty_outputs) < len(outputs):
                logger.warning(f"WARNING: {len(outputs) - len(non_empty_outputs)}/{len(outputs)} outputs are empty or whitespace only. Sample empty output: '{outputs[0][:50] if len(outputs[0]) > 50 else outputs[0]}'")
            if len(outputs[0].strip()) == 0:
                logger.warning(f"First output is empty after strip. Raw: '{outputs[0]}', Generated IDs shape: {generated_ids.shape}, First 10 token IDs: {generated_ids[0].cpu().tolist()[:10] if generated_ids.shape[-1] > 0 else 'empty'}")
